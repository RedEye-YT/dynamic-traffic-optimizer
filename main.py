...
# COCO IDs: 1:bicycle, 2:car, 3:motorcycle, 5:bus, 6:train, 7:truck
TARGET_CLASSES = [1, 2, 3, 5, 6, 7]
...
while cap.isOpened():
    ...
    # Run YOLO Tracking strictly for our target vehicles
    results = model.track(frame, persist=True, classes=TARGET_CLASSES)

    # SAFETY CHECK for tracking IDs
    if results[0].boxes is not None and results[0].boxes.id is not None:
        ...
        for box, track_id, class_idx in zip(boxes, track_ids, class_indices):
            ...
            # Style to match reference image: Green Box, Green Text, Red Dot
    ...
    # Display the Object Counts on the Top Left (Green Text)
    y_offset = 30
    for class_name, count in class_counts.items():
        cv2.putText(frame, f"{class_name}: {count}", (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        y_offset += 30
